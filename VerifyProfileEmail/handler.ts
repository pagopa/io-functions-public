...
export function VerifyProfileEmailHandler(
  ...
  return async (context, token) => {
    ...
    if (isLeft(errorOrMaybeTableEntity)) {
      context.log.error(
        `${logPrefix}|Error searching verification token|ERROR=${errorOrMaybeTableEntity.value.message}`
      );
    ...
    // Check if the entity is a VerificationTokenEntity
    const errorOrVerificationTokenEntity = VerificationTokenEntity.decode(
      maybeTokenEntity.value
    );

    if (isLeft(errorOrVerificationTokenEntity)) {
      context.log.error(
        `${logPrefix}|Verification token can't be decoded|ERROR=${readableReport(
          errorOrVerificationTokenEntity.value
        )}`
      );
      return ResponsePermanentRedirect(
        verificationFailureUrl(
    ...
    const verificationTokenEntity = errorOrVerificationTokenEntity.value;
    const {
